...
const MyAdminSalon = () => {
    ...
    const fetchQrCode = async (id) => {
        if (!id || qrCodeImage) return;
        try {
            setQrLoading(true);
            const blob = await getSalonQrCodeApi(id);
                    ...
                    <Reveal>
                        <div className="max-w-2xl mx-auto">
                            <SectionHeading 
                                ...
                                title="Your Salon QR" 
                                description="Your official salon QR code. Display this image for customers to scan and book appointments instantly."
                                align="center"
                                mb="mb-10"
                            />

                            {/* Main QR Display */}
                            <div className="bg-white rounded-[40px] shadow-2xl overflow-hidden border border-gold/10 p-6 sm:p-10 flex flex-col items-center">
                                <div className="relative w-full aspect-[1/1] max-w-[500px] bg-beige/30 rounded-3xl overflow-hidden flex items-center justify-center border border-gold/5 shadow-inner">
                                    {qrLoading ? (
                                        <div className="flex flex-col items-center gap-4">
                                            <div className="w-16 h-16 border-4 border-gold/20 border-t-gold rounded-full animate-spin" />
                                            <p className="text-xs text-gold font-bold uppercase tracking-widest animate-pulse">Fetching Image...</p>
                                        </div>
                                    ) : (
                                        <img 
                                            src={qrCodeImage || salon.qrCodeUrl} 
                                ...
                                {/* Actions */}
                                <div className="mt-10 grid grid-cols-1 sm:grid-cols-2 gap-4 w-full max-w-[400px]">
                                    <button 
                                        onClick={() => {
                                            const url = qrCodeImage || salon.qrCodeUrl;
                                            if (!url) return;
                                            const link = document.createElement('a');
                                            link.href = url;
                                            link.download = `${salon.name}_QR.png`;
                                            document.body.appendChild(link);
                                            link.click();
                                            document.body.removeChild(link);
                                        }}
                                        className="px-8 py-4 rounded-full bg-gold text-black-deep text-[11px] font-bold uppercase tracking-widest hover:bg-gold/80 transition-all flex items-center justify-center gap-2 shadow-lg"
                                    >
                                    ...
                                    <button 
                                        onClick={() => {
                                            const qrImageUrl = qrCodeImage || salon.qrCodeUrl;
                                        ...
                                        className="px-8 py-4 rounded-full bg-black-deep text-gold text-[11px] font-bold uppercase tracking-widest hover:bg-black transition-all flex items-center justify-center gap-2 shadow-lg"
                                    >
                                        <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2.5}>
