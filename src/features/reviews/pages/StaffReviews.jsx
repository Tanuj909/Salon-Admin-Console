...
const StaffReviews = () => {
    ...
    const fetchReviews = async (sid, p) => {
        try {
            const reviewsData = await getReviewsByStaffApi(sid, { page: p, size: pageSize });
        ...
        return (
            <div className="w-full flex items-center justify-center min-h-[400px]">
                <div className="flex flex-col items-center justify-center gap-4">
                    ...
                    <span className="text-secondary font-medium tracking-wider uppercase text-sm">Loading My Reviews...</span>
                </div>
            </div>
        );
    }
            ...
            <div className="mx-auto px-4 md:px-6 pb-12 pt-4 bg-transparent max-w-5xl">
                ...
                {reviews.length === 0 ? (
                    <div className="bg-white rounded-[24px] py-24 px-8 text-center border border-slate-100 shadow-sm">
                        ...
                        <p className="text-secondary max-w-sm mx-auto">When customers leave feedback for your services, it will appear here.</p>
                    </div>
                ) : (
                    <div className="grid gap-6">
                        {reviews.map((review) => (
                            <div key={review.id} className="bg-white rounded-xl p-5 shadow-sm border border-slate-100 transition-all hover:shadow-md hover:border-gold/30 group">
